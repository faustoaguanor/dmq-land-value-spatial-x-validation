...
# ROOT apunta al repositorio oficial (de solo lectura: aqui solo se leen los
# CSV ya calculados, nunca se escribe nada de vuelta en el repositorio original).
ROOT      = Path(os.environ.get("TESIS_REPO", OUT_DIR.parent.parent.parent.parent / "proyecto" / "replicacion"))
ANALISIS  = ROOT / "analisis" / "output_log"

# Correccion observacion 1.1: los datos internos siguen usando la clave "SANNWR"
# (asi estan en los CSV de analisis/output_log/, cambiarla rompe el filtrado por
# PRINCIPAL/COLOR_MODEL), pero toda etiqueta que el lector VE debe decir
# "SANNWR-adaptado" -- la adaptacion combina las distancias con pesos fijos
# 0,5/0,5, no aprendidos como en el diseno publicado por Ni et al. (2022).
DISPLAY_ALIAS = {"SANNWR": "SANNWR-adaptado"}
def _display(nombres):
    return [DISPLAY_ALIAS.get(n, n) for n in nombres]
...
def figure1():
    ...
    ax.set_yticklabels(_display(modelos), fontsize=10)
    ax.invert_yaxis()
...
def figure2():
    ...
    for ax, (rkf_vals, sb_vals, ylabel, title, ylim) in zip(axes, [
        ...
        ax.set_xticks(x)
        ax.set_xticklabels(_display(MODELS_CV), fontsize=10, rotation=0)
...
def figure3():
    ...
    ax.set_xticks(x)
    ax.set_xticklabels(_display(modelos), fontsize=11, rotation=20, ha="right")
