...
import os
# ROOT (lectura): repositorio oficial, de solo lectura -- aqui solo se leen
# predicciones ya calculadas. OUT_DIR/FIGS_DIR (escritura): SOLO dentro de este
# repositorio nuevo, nunca de vuelta en el original.
ROOT      = Path(os.environ.get("TESIS_REPO", Path(__file__).parent.parent.parent.parent.parent / "proyecto" / "replicacion"))
OUT_DIR   = Path(__file__).parent / "_out_analisis"
FIGS_DIR  = Path(__file__).parent
OUT_DIR.mkdir(exist_ok=True)
FIGS_DIR.mkdir(exist_ok=True, parents=True)

# Correccion observacion 1.1: SANNWR (el canonico Ni 2022 con la adaptacion de
# pesos fijos) se rotula SANNWR-adaptado en toda leyenda. SANNWR* (la variante
# de grilla, un modelo distinto) no se toca.
...
def run_t3_cdf_por_decil(all_preds, smeared_df):
    ...
    for modelo, color in zip(modelos, colors):
        ...
        ax.plot(err, cdf, label=DISPLAY_ALIAS.get(modelo, modelo), color=color, lw=1.8, alpha=0.85)
    ...
    piv = piv.rename(columns=DISPLAY_ALIAS)
